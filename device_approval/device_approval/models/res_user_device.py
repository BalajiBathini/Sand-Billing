...
class ResUserDevice(models.Model):
    ...
    def _send_status_notification(self, status):
        """Send email notification to user about device status change."""
        template_map = {
            'approved': 'device_approval.mail_template_device_approved',
            'revoked': 'device_approval.mail_template_device_revoked',
        }
    ...
    def _notify_admin_new_device(self, device):
        """Notify administrators about a new pending device."""
        template = self.env.ref(
            'device_approval.mail_template_device_pending', raise_if_not_found=False)
        if template:
            template.send_mail(device.id, force_send=True)
